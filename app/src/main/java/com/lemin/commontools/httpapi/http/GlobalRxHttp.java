...
import com.lemin.commontools.httpapi.interceptor.AddCookiesInterceptor;
import com.lemin.commontools.httpapi.interceptor.CacheInterceptor;
import com.lemin.commontools.httpapi.interceptor.HeaderInterceptor;
import com.lemin.commontools.httpapi.interceptor.ReceivedCookiesInterceptor;
...
//                    LogHelper.error("HttpLog", message);
                    logMessage(message);
                }
            });
    ...
    private void logMessage(String xml) {

        if (xml.startsWith("{")) {
//            DD.json(xml);
        } else {
            LogHelper.error("GlobalRxHttp：", xml);
        }
    }
    ...
    public GlobalRxHttp setCache() {
        ...
        Cache cache = new Cache(new File(Environment.getExternalStorageDirectory().getPath() + "/beenmanage")
                , 1024 * 1024 * 100);
